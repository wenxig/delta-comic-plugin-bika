...
const userCharactersTranslator = (character: string) => Object.hasOwn(bikaCharTranslateMap, character) ? bikaCharTranslateMap[character] : character
</script>

<template>
  <NThing class="bg-(--van-background-2) overflow-hidden relative w-full">
    <template #avatar>
      <Comp.Image :src="user.avatar" :fallback="userIcon" previewable fit="cover" class="size-16 mt-1 ml-1" round />
    </template>
    <template #header>
      <div class="mt-2 -mb-2 flex items-center">
        {{ user.name }}
        <div class="flex mx-1 items-center text-(--nui-primary-color)" v-if="user.customUser.gender == 'm'">
        ...
        <div class="flex mx-1 items-center text-(--nui-primary-color)" v-else-if="user.customUser.gender == 'f'">
          <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="w-4"
            viewBox="0 0 1024 1024">
            <path
        ...
        <span class="mr-1 text-xs text-(--nui-primary-color) font-normal">Lv{{ user.customUser.level }}</span>
      </div>
    </template>
    <template #description>
      ...
      <div class="flex !w-[60%] items-center">
        <span class="mr-1 no-color-change-transition text-xs text-(--van-text-color-2)">
          {{ exp }}/{{ needExp }}
        </span>
    ...
    <Comp.Text :text="user.customUser.slogan" v-if="!isSmall"
